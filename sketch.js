...
function preload(){
  ...
  run = loadImage("images/download.png");
...
function animate(spritesheet,spritedata,startFrame,endFrame,xpos,ypos,width,height,loop,speed){
   var animation = [];
   ...
   if(loop){
    for(var i = startFrame; i<endFrame; i++){
     ...
     if(typeof pos ==="object"){
      animation[i] = spritesheet.get(pos.x,pos.y,pos.width,pos.height);
      console.log(animation);
   }
  }
    image(animation[(frameCount%15)+30],xpos,ypos,width,height);
  }
   else{
    for(var i=startFrame;i<startFrame+100; i++){
      if(i<endFrame-startFrame){
        var pos = frames[0];
        animation[0] = spritesheet.get(pos.x,pos.y,pos.width,pos.height);
        image(animation[0],xpos,ypos);
      }
    }
   }
}